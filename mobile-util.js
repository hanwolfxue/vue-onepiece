...
window.mobileUtil = (function(win, doc) {
  ...
  return {
     ...
     */
    fixScreen: function() {
      var metaEl = doc.querySelector('meta[name="viewport"]'),    //meta元素
        metaCtt = metaEl ? metaEl.content : '',   //meta的content内容
        matchScale = metaCtt.match(/initial\-scale=([\d\.]+)/),   ///meta的缩放
        matchWidth = metaCtt.match(/width=([^,\s]+)/);    //meta的宽度

      if ( !metaEl ) { // REM
        var docEl = doc.documentElement,
          maxwidth = docEl.dataset.mw || 750, // 每 dpr 最大页面宽度
          dpr = isIos ? Math.min(win.devicePixelRatio, 3) : 1,    //只处理ios系统
          scale = 1 / dpr,
          tid;
        ...
        var	width = parseInt(matchWidth[1]),
          ...
          w = Math.min(iw,ow,sw,saw,ih,oh,ish,sah),   //求出最小的宽度
          scale = w / width;    //求出缩放比，设备宽度/布局视口宽度
